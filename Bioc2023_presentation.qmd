...
- `dualFilter`, `mergeByHMP`, `mergeByCol`, `mergeBySig`
...
```{r setup-extra}
#| code-line-numbers: '1-4|6-13|14-20|22-29'
...
- Can also return only ranges *covered* by $>p\times n$ samples (between red lines)
...
```{r union-peaks}
#| code-line-numbers: '1|2-7|8|1-9'
#| output-location: fragment
union_peaks <- makeConsensus(treatment_peaks, var = "centre") %>% 
  mutate(
    centre = paste0(
      seqnames, ":", round(vapply(centre, mean, numeric(1)), 0)
    ) %>% 
      GRanges(seqinfo = sq)
  ) %>% 
  select(centre)
