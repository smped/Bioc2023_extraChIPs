...
- Post-doctoral Bioinformatician at Black Ochre Data Laboratories
...
- Seamless integration with the `tidyverse` [@tidyverse2019] + `plyranges` [@plyranges]
...
- Simplify common processes:
    + Detection of Differential Signal (Fixed-Width or Sliding Windows)
...
- `reduceMC`, `intersectMC`, `setdiffMC`, `unionMC` etc
...
#### Working with Peaks/GRangesList
...
- `dualFilter`, `mergeByHMP`
- `mergeByCol`, `mergeBySig`
...
- `plotPairwise` ^1^, `plotProfileHeatmap`
- `plotHFGC`
...
::: aside
^1^ Only available in `devel`
:::

::: {.notes}
Functions higlighted with an asterisk are only in `devel`
:::
...
- Seqinfo objects can also be specified
...
## Demonstration Data ^[Taken from SRA BioProject PRJNA509779 and subset to a region of chr10]

```{r samples}
#| output-location: column-fragment
#| code-line-numbers: '1-11|12-19|20-22'
## Define Samples
treat_levels <- c("E2", "E2DHT")
samples <- tibble(
  accession = paste0("SRR83151", seq(80, 91)),
  target = rep(c("ER", "H3K27ac"), each = 6),
...
sample_colours <- setNames(
  treat_colours[samples$treatment], 
  samples$accession
)
...
```{r setup-extra}
#| code-line-numbers: '1-3|4-9|10-1'
sq <- Seqinfo(
...
- Import broad/narrowPeak files from `macs2 callpeak` or another peak-caller
...
- Can also return only ranges *covered* by `p` samples
- Any/all mcols can be returned
...
- Multiple visualisation functions after counting reads
    + `plotAssay*` for `SummarizedExperiment` objects
...
```{r count-fixed}
#| code-line-numbers: '3-6|7-14|15-17|18-22|23-26|1-27'
#| output-location: fragment
library(Rsamtools)
library(csaw)
...
centred_peaks <- union_peaks %>% 
  mutate(
    centre = paste0(seqnames, ":", centre), # Setup centred ranges
    union_peak = granges(.) # Add the original ranges to mcols
  ) %>% 
...
```{r count-sliding}
#| code-line-numbers: '1-5|6-9|10-17|1-18'
#| output-location: fragment
## Define the BamFile List INCLUDING the Input
acc <- unique(c(samples$H3K27ac$accession, samples$H3K27ac$input))
...
- Only retain windows which are likely to contain signal
...
## plotSplitDonut {.scrollable}

```{r plot-donut-custom, fig.height=6.5, fig.width=9}
#| output-location: slide
#| code-line-numbers: '1-7,9-10|11-12|13|14'
...
- Only show labels for rare events
...
## mapGrlCols {.scrollable}

- Multiple easy options for downstream analysis

```{r plot-er-h3k-donut, fig.height=6.5}
#| output-location: slide
...
all_results %>% 
  ...
  plotSplitDonut(
    ...
    explode_r = 0.3, min_p = 0, label_alpha = 0.8
  )
```
...
## plotPairwise: logFC {.scrollable}

```{r plot-pairwise-logfc, fig.height=6.5}
