...
knitr::opts_chunk$set(
  ...
  fig.height = 6, fig.width = 8, dev = 'png', crop = NULL
)
```
...
pkg <- c(
  "rtracklayer", "tidyverse", "glue", "plyranges", "Rsamtools", "csaw", "edgeR",
  "patchwork"
)
...
All other functions behave in a simlar manner
...
```{r import-peaks}
fl <- glue("data/{er_samples$accession}_peaks.narrowPeak")
...
Blacklisted regions and greylisted regions are also able to be applied during import.
...
These have also been provided and can be loaded separaetly then combined.

```{r black-grey-lists}
bg_list <- c("data/chr10_blacklist.bed", "data/chr10_greylist.bed") %>% 
...
```{r load-gtf}
gtf <- file.path("data", "gencode.v43lift37.chr10.annotation.gtf.gz") %>% 
  import.gff() %>% 
  split(.$type)
seqinfo(gtf) <- sq
```
...
```{r load-er-counts}
er_bfl <- BamFileList(glue("data/{er_samples$accession}.bam"))
names(er_bfl) <- er_samples$accession
...
A key advanatge of `plotSplitDonut()` is the ability to specify inner and outer palettes separately
...
h3k_bfl <- BamFileList(glue("data/{h3k_samples$accession}.bam"))
names(h3k_bfl) <- h3k_samples$accession
```
...
```{r filtcounts}
guide_ranges <- import.bed("data/H3K27ac_chr10.bed", seqinfo = sq)
...
```{r plot-filt-assays, fig.cap = "A) Density plot and B) PCA based on logCPM values from the sliding windows retained after filtering."}
A <- plotAssayDensities(filtcounts, assay = "logCPM", colour = "treatment") + 
  scale_colour_manual(values = treat_colours)
B <- plotAssayPCA(filtcounts, assay = "logCPM", colour = "treatment", label = "accession") +
  scale_colour_manual(values = treat_colours)
...
```{r merge-h3k}
h3k_results <- mergeByHMP(h3k_all, min_win = 2) %>% 
  addDiffStatus()
h3k_results
```
...
Next to these is the column `keyval_range` which indicates the original window which returned the lowest p-value.
Given the harmonic men involves summing the inverse of raw p-values, these are considered to be weights and the returned values for logCPM and logFC are weighted averages, using $w_i = \frac{1}{p_i}$ as weights for each windows $i$.
...
```{r key-results}
both_results %>% 
  mapGrlCols(var = c("region", "status"), collapse = "gene_name") %>% 
  filter(
